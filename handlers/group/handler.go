...
func (r *resource) CreateGroup(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&group); err != nil {
		customErr := customError.CustomErrorFactory(customError.WithBadRequest(err.Error(), "Invalid request body"))
	...
	err := group.Validate()
	if err != nil {
		customErr := customError.CustomErrorFactory(customError.WithBadRequest(err.Error(), "Validation error"))
		c.JSON(customErr.Status, customErr)
		return
	}
...
func (r *resource) GetGroup(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		customErr := customError.CustomErrorFactory(customError.WithBadRequest("Group id is empty", "Invalid request params"))
		c.JSON(customErr.Status, customErr)
	}
...
func (r *resource) GetMyMatch(c *gin.Context) {
	...
	if username == "" {
		customErr := customError.CustomErrorFactory(customError.WithBadRequest("Username is required", "Validation error"))
		c.JSON(customErr.Status, customErr)
		return
	}
